...
### 3.3 ⚠️ `switchMs` **不随尺寸单调** —— 根因已定位，见后续文档
...
**5MB 比 10MB 更慢**，旧构建（v1.5.9）下同样得到 5MB ≈ 2905/2835ms，两次独立构建都复现。

> **根因已定位（同日稍后）**：这是**大文件模式（Large File Mode）的阈值悬崖**，
> 方向是「更小的文档更慢」。`LARGE_FILE_BYTES_THRESHOLD = 5 × 1024 × 1024` 且判定用
> **严格大于**，而夹具 `5MB.md` 的字节数**恰好等于** 5,242,880 → **不触发**降级 →
> 对 5MB 文档付全量渲染成本；`10MB.md` 触发降级（视口裁剪、装饰上限、关拼写/动画）→ 更快。
> 同时 `switchMs` 的原判据（绝对阈值 60 ≈ 0.4% 采样点）**不足以支撑结论**，已改为
> 「帧对变化比例 ≥3%」。详见 `2026-09-25-hotopen-criterion-and-large-file-cliff.md`。
>
> 本文 §3.1/3.2 的**数值本身可信**（与修正判据后的 2917/2294ms 吻合），
> 但「不得归因于大文件处理」这一措辞已被更正为「确实存在阈值效应，只是方向反直觉」。
