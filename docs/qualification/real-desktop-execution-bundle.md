...
> CI runner 已证明：三平台构建+启动+渲染 PASS（见 runtime-matrix-evidence-2026-08-18.md）。Linux 输入改用 Weston headless Wayland 的 virtual keyboard 通道，避免 Xvfb/XTEST 不能抵达 WebKitGTK 的已知限制；以当前候选提交的 workflow 结果为准。
...
### 3.1 Linux（GitHub Actions：Weston Wayland + fcitx5）—— 中文 IME 矩阵（8 场景，自动断言）
```bash
# workflow 已安装依赖；以下仅用于在 Linux CI job 内复现
sudo apt install -y weston wtype fcitx5 fcitx5-chinese-addons fonts-noto-cjk   # Ubuntu/Debian
# 或：sudo dnf install -y weston wtype fcitx5 fcitx5-chinese-addons              # Fedora
...
# CI 启动 Weston headless Wayland 与 fcitx5 后执行矩阵；wtype 走 virtual-keyboard 协议。
export WAYLAND_DISPLAY=wayland-0 GDK_BACKEND=wayland GTK_IM_MODULE=fcitx QT_IM_MODULE=fcitx XMODIFIERS=@im=fcitx
...
同日完整回归（隔离 release、单次连续执行 Journey 1、2、3、4、7、8、9、10、15、17）全部通过：Latin、中文 IME、日文 IME（兼容性观察）、选择加粗、列表 Enter、表格 Tab、数学/ Mermaid 源码保真、Undo、10 MB 编辑保存均为 PASS。10 MB 本轮 `editable=621 ms`、`editSave=5779 ms`。该结果是 Mellow 自身 macOS 回归证据；Windows/Linux CI Runtime Qualification 仍须以当前候选提交重新执行，不能由该本机结果替代。
...
同日，在用户明确关闭 Typora 后，以 `--app=both --close-existing-typora --journey=1,2,3,4,7,8,9,10,15,17` 执行了同一份临时文件的双端对照，runner 已确认 Typora 为 1.14.9（build 7785）。两端均通过 Latin input、中文 IME、选区加粗、列表 Enter 续写、表格 Tab 导航、数学／Mermaid 源码保真及 Undo。日文 IME 因本机未启用日文罗马字输入源而在两端均为 SKIP；它现为非阻断性兼容性观察。10 MB 文件上，Mellow 完成编辑、保存和读回（总旅程 9.094 s，编辑与保存 5.366 s）；Typora 在 200.753 s 后提示“该文件过大，无法呈现”。后者是 Typora 的产品边界，不是 Mellow 失败，Mellow 在该项为更优实现。此对照只补强 macOS 证据，不能替代 Windows/Linux CI 或完整 UX Gate。
...
## 五、已知边界（CI 已定位）
- Linux Xvfb 下 WebKitGTK 不接收 XTEST 合成键；CI 已切换为 Weston Wayland + `wtype` virtual keyboard，不再用该无效通道判定 IME；
- Windows CI 无交互桌面，SendKeys 未达 WebView2，因此其保存读回仍仅作诊断。
