...
| **G7-MENU-06** | Edit 菜单缺 `New Paragraph` / `New Line` | Typora Edit 菜单含这两项（Enter / Shift+Enter） | 无对应菜单条目 | **FAIL → 已复核（2026-09-13），登记为依赖 G7-EDIT-07**。<br/>**一手证据**：官方 Shortcut Keys 页（2026-09-06 更新）Edit 段开头确为 **New Paragraph（Enter）/ New Line（Shift+Enter）**，顺序在 Cut 之前 —— 原判定成立。<br/>**但不立即补菜单项**：实测 Mellow 的 **Shift+Enter 与 Enter 完全等价**（均产出 `abc\ndef`），见 G7-EDIT-07。此时加入「New Line」会得到一个**名为换行、实为分段**的误导性入口，比缺失更损害一致性。故两项合并登记，待软换行能力落地后**一并**加入（届时才真正等价）。 |
...
| **G7-EDIT-07（新，2026-09-13 实测）** | **Shift+Enter 与 Enter 完全等价：Mellow 没有软换行** | **FAIL（真实行为差距，方案此前未记录）** —— 实测（Playwright，光标置于行尾）：`abc` + Enter → `"abc\ndef"`；`abc` + Shift+Enter → `"abc\ndef"`，**产出完全一致**。而 Typora 的 Shift+Enter 插入**行内换行**（`<br>`，不分段）。<br/>根因：`lang-markdown` 只绑定 `{key:"Enter", run: insertNewlineContinueMarkup}`；**全仓无 `Shift-Enter` 绑定**，且 `editor-engine` 与 vendored CoreEditor **均无 `HardBreak` 节点处理** —— 即便写入 Markdown 硬换行（`  \n` / `\`）也无从渲染。<br/>**为何本轮不直接实现**：① 需要补硬换行的解析 / 渲染（Live 引擎现无此节点），属新能力而非对齐；② 输入路径是最高风险面（与 D-V 同判据），一次回归即「打字坏了」级别事故。<br/>**影响**：`G7-MENU-06` 的「New Line」菜单项因此被阻塞（见 §5.1）。 |
