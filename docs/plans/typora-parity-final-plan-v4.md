...
| 2026-09-05 | V4.5 | 桌面对标第三轮（用户确认 E1–E6 全量 + StatusBar 收敛默认集 + 发版 v1.4.5）：基于 Typora 1.14.6–1.14.9 增量审计（Explore 10 线核查 + 菜单 dump），确认缺口为常驻工具栏/Readonly Mode/编辑体验细节；E1 常驻编辑器工具栏（`EditorToolbar.tsx` 13 键 + View→Toolbar `view.toolbar.toggle`，默认隐藏对齐 Typora）；E2 图片悬浮操作条首位"尺寸"按钮（W×H prompt + 百分比）+ 尺寸徽标（`mellow-md-image-size-badge`，显示 缩放后尺寸·百分比）；E3 代码块语言标签（起始行行尾大写标签 + 点击下拉换语言改写 info string，`codeBlockLabel.ts`）+ 围栏块/`$$…$$` 数学块编辑态视觉描边（`mellow-md-editing-outline` 行装饰）；E4 源码模式行号独立偏好 `editor.sourceLineNumbers`（Live 默认关/Source 默认开，`sourceMode.toggle` 按 `__MELLOW_LINE_NUMBER_PREFS__` 应用）；E5 菜单去重（移除 Insert→Alert 与 Slash 重复 insert.table，保留命令供 slash/palette）；E6a 只读模式（`readonly.ts` editable/readOnly Compartment + `.mellow-readonly` + View→只读模式切换，requestMeasure write 阶段落 class 修 ctor dispatch 限制）；E7 StatusBar 收敛默认集（dirty/markdown/encoding/eol/zoom/status 默认隐藏，`fieldVisible` 纯函数导出）；e2e image-upload 预置改字段级 key（`mellow.statusbar.fields`）；全量回归：engine 1121 + 各包 73 jest 全绿、12 守卫 OK、8 e2e PASS、Golden OK；发 v1.4.5 |
| 2026-09-05 | V4.6 | 桌面对标第四轮（用户确认 A 组全量 + B2/B3 + C 小项 + 发版 v1.4.6）：桌面 UI/布局完全对标 + 功能/体验/侧边栏/特点/菜单 收口；A1 写作限宽内部化到 iframe `.cm-content`（`setContentMaxWidth` 全链路 `CoreEditor config→styling→bridge→editor-core→App`，CSSStyleDeclaration 走 `setProperty('…','important')` 压过 MarkEdit 主题，frame 通栏、滚动条贴窗缘）；A2 窗口几何记忆 Rust 自研持久化（`geometry.rs` 模块 + `app_data_dir/window-geometry.json` + 屏幕外有效性校验 ≥120×48px + `on_window_event` Moved/Resized/Destroyed + `RunEvent::ExitRequested/Exit` flush，3/3 单测绿）；A3 Tabbar `+` 新建钮 / 双击空白区新建 / 中键关 tab（`packages/desktop-ui/src/Tabbar.tsx` 扩展 + `tabbar.test.tsx` 3/3 绿）；B2 停用欢迎页对齐 Typora（删除 `Welcome.tsx` + 死导出 + i18n 6 孤儿键，启动即文档）；B3 窗口顶层菜单仅 macOS（`menuSchema.ts` window root `macOnly:true` + `menu-schema.test.ts` 不变量 + `verify-menu-contract.mjs` macOnly 集合断言，Typora Windows/Linux 真机 dump 验证）；C1 另存建议文件名（`documentSuggestedName` 取首个 # 标题或首非空行，整图删除 + 非法文件名字符过滤 + 48 字截断，4/4 jest 全绿）；C2 会话恢复审计：Typora 1.14.9 无 session restore（macOS 走系统文档自动保存）→ no-op（记录决策）；C3 Help 结构：现有 `help.quickStart/markdownReference/cheatsheet/feedback` 四命令已在 CommandRegistry 闭环 + Menu schema 命令集通过契约（无需新实现）；验收：commands 25 + app-core 204 + desktop-ui 23 jest 全绿、Rust lib 61 + integration + updater 全绿、13 静态守卫（含 `verify-menu-contract-guard.mjs` 18 变异注入）全绿、Visual Golden 6 配置（900/1200/1440/zoom-200/dark-900/dark-1440）重刷后 ±1px 自对比匹配；发 v1.4.6 |
